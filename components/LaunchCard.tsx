...
export function LaunchCard({ card, attendee } : { card : iCard; attendee : iAttendee; }) {
  return <Link to={`/launches/${card.launchId}/cards/${card.id}`} className='launch-card rounded border border-dark d-flex flex-column p-1 text-center cursor-pointer'>
    <AttendeeInfo className='flex-grow-0 mr-1 font-weight-bold border-bottom pb-1 mb-1' hidePhoto attendee={attendee} />
